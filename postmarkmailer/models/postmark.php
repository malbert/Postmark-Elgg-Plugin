...
class Postmark {
    ...
    function Postmark($params = array())
    {
        ...
        elgg_log('Postmark Class Initialized','DEBUG');
    ...
    */
    ...
    function send($from_address = null, $from_name = null, $to_address = null, $to_name = null, $subject = null, $message_plain = null, $message_html = null)
	{
        ...
        if (!function_exists('curl_init'))
        {
        	...
        	elgg_log('Postmark - PHP was not built with cURL enabled. Rebuild PHP with --with-curl to use cURL.', 'ERROR');            
            return false;       
        }
		...
		if (is_null($this->api_key)) {
			elgg_log("Postmark API key is not set!", 'ERROR');            
            return false; 
		}
		
		if (is_null($this->from_address)) {
			elgg_log("From address is not set!", 'ERROR');            
            return false; 
		}
		
		if (is_null($this->_to_address)) {
			elgg_log("To address is not set!", 'ERROR');            
            return false; 
		}
		
		if (is_null($this->_subject)) {
			elgg_log("Subject is not set!", 'ERROR');            
            return false; 
		}
		
		if (is_null($this->_message_plain) && is_null($this->_message_html)) {
			elgg_log("plain message or hmtl or both should be set!", 'ERROR');            
            return false; 
		}
		...
		$return = curl_exec($ch);
		elgg_log('POSTMARK JSON: ' . $encoded_data . "\nHeaders: \n\t" . implode("\n\t", $headers) . "\nReturn:\n$return", 'DEBUG');
		
		if (curl_error($ch) != '') {
			elgg_log(curl_error($ch), 'ERROR');            
            return false; 
		}
		
		$httpCode = curl_getinfo($ch, CURLINFO_HTTP_CODE);
		elgg_log('POSTMARK http code:' . $httpCode, 'DEBUG');
		
		if (intval($httpCode / 100) != 2) {
			$message = json_decode($return)->Message;
			elgg_log('Error while mailing. Postmark returned HTTP code ' . $httpCode . ' with message "'.$message.'"', 'ERROR');
			return false;
		}
		return true;
	}
